...
def fit_bb(dense_lc,wvs):
    ...
    for i,datapoint in enumerate(dense_lc):
        ...
        fnu = 10.**((datapoint[:,0] + 48.60)/-2.5)
        fnuplus = 10.**((datapoint[:,0] + datapoint[:,1] + 48.60)/-2.5)
        fnu = fnu * 4. * np.pi * (3.086e22)**2 #LAZZYYYY assumption of 10 kpc
        fnuplus = fnuplus * 4. * np.pi * (3.086e22)**2
        ...
        plt.plot(wvs,flam,'ko')
        plt.plot(wvs,bbody(wvs,T1,R1),'ro')
        plt.show()
        '''
